...
- Codex 外观调整由 `CodexAppearanceConfig` 持久化在 Codey 自己的 `config.json` 中，字段为 `backgroundDataUrl`、`backgroundFileName`、`backgroundOpacity`、`surfaceOpacity` 和 `chatWidth`。图片限制为 `data:image/*` 数据 URL，后端保存前限制长度，前端选择图片时先缩放到最长边 2400px 并转为 JPEG；没有图片时不接管旧版外部自定义背景，保证升级兼容。
- `public/codex-appearance.js` 作为 `codex-appearance` 内置 CDP 脚本注入。背景层只按 `#root .thread-scroll-container` 或稳定的中央内容 surface 定位，并把顶部边界裁到精确的 `.top-toolbar-sm` 底部；不得使用包含 `top-toolbar` 子串的宽泛选择器，否则会把侧栏 resize handle 当成顶栏，导致背景错误地出现在工具栏或标题区域。对话宽度通过 `--thread-content-max-width` 设置，遮罩只改变内容 surface 的透明度，侧栏、导航、顶部工具栏保持不显示背景图片。
- 外观控制器监听 `codey:config-changed`，保存配置后可以热应用；`ResizeObserver`、`MutationObserver` 和有界重试只负责重新计算中央区域边界。控制器自身保存 `window.__codeyCodexAppearance` 版本标记，并在销毁时移除监听器，避免健康重注入后叠加 watcher。旧版 `codex-customizer.js` 检测到 Codey 已接管背景后会停止绘制并移除旧按钮/面板，避免两个控制器相互覆盖；当 Codey 没有设置新背景时，旧版控制器仍可继续工作。
...
### GitHub Actions 上游同步与本地功能保留

远程 Fork 的正式同步入口是 `.github/workflows/sync-upstream.yml`，每两天在 GitHub 的 Windows Runner 上运行；本地不再承担定时同步。Workflow 默认把 `https://github.com/SuperGness/codey.git` 作为上游，把当前 Fork 作为 origin，并调用仓库中的 `scripts/sync-upstream.ps1` 完成安全合并。

同步前会保留 GitHub 工作区的完整历史和 bundle 备份；只使用普通 merge，不执行 `reset`、`clean`、自动 stash 或静默的 ours/theirs 选择。合并冲突时 Workflow 失败并创建一个待处理 Issue，同时上传 `.codex-sync/last-sync.json`；不会把冲突状态推回 `master`。合并成功后会触发 `build-windows-installer.yml`，构建新的 Windows 安装包并上传为 Action Artifact。

首次从无共同历史的本地快照迁移到 Fork 时，只需要完成一次远程基线提交。基线建立后，后续改动直接在 GitHub 上提交或通过 Pull Request 完成；Windows 安装包 Workflow 会在 `master` 更新时自动构建，也可以从 Actions 页面手动运行。`.codex-sync/` 已加入忽略规则，不会污染提交。
...
未配置上述 variable 或 secret 时，现有 GitHub Release 发布不受影响，R2 同步会被跳过。默认构建使用项目公开的 R2 更新源；设置 `CODEY_UPDATE_BASE_URL` 可以在编译时覆盖该地址。配置页面不允许用户改写更新源。检查更新会经 HTTPS 拉取清单，校验版本、下载地址和 SHA-256 格式后显示是否有新版本；同一清单 URL 的检查结果缓存 30 秒，下载命令可复用 10 分钟内已验证的候选，网络或解析失败不写缓存，因而页面先检查再下载不会重复拉取清单。Codey 只在 Codex runtime 已成功启动后执行自动更新检查：检查超过 300 毫秒才显示无按钮的原生状态窗，10 秒硬超时、网络错误或清单错误均关闭提示并继续运行，更新检查绝不阻塞 Codex 首次启动。Windows 状态窗运行在独立 Win32 消息线程；macOS 主线程运行 AppKit 事件循环，Tokio runtime 移到工作线程，状态窗使用不激活 Dock 图标的 `NSPanel`。发现当前平台可安装的新版本时使用原生自定义按钮询问；选择稍后会把本次结果保存在 `AppState`，renderer 从 `/backend/status` 恢复 Codey 图标红点，本次运行不再强弹，后续每 30 分钟只静默刷新红点。确认更新后复用同一次检查已验证的资产信息，显示下载校验状态，最长等待 300 秒；安装器成功拉起后请求正常的更新关闭流程，停止受控 Codex 并释放可替换文件。下载、校验或安装器启动失败时保留当前 Codex 会话。当前 macOS 包仍是未签名包，Windows 包也尚未进行代码签名，因此不会静默下载或安装。
